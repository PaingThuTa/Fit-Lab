    ...
    <Card title="Enrolled members" description="Snapshot of who is in your programs">
      ...
      <div className="space-y-3 md:hidden">
        {trainerEnrollments.map((enrollment) => {
          ...
          return (
            <div key={enrollment.id} className="mobile-list-row">
              <p className="font-medium text-slate-900 dark:text-white">{enrollment.memberName}</p>
              <p className="text-sm text-slate-600 dark:text-slate-300">{enrollment.courseName || course?.title || 'Removed course'}</p>
              <p className="mt-1 text-xs text-slate-500">Enrolled {formatShortDate(enrollment.enrolledAt)}</p>
      ...
      {trainerEnrollments.length === 0 ? (
        <p className="mt-3 status-muted">No member enrollments for your courses yet.</p>
      ) : null}
    </Card>
  )
}
